    ...
    public function getAccessToken()
    {
        if (is_null($this->_accessToken)) {
            if (empty($this->access_token)) {
                $this->errorServer(Yii::t('oauth2', 'The request is missing "access_token" parameter'));
            }
            if (!$this->_accessToken = AccessToken::findOne(['access_token' => $this->access_token])) {
                $this->errorServer(Yii::t('oauth2', 'The Access Token is invalid'));
            }
        }
